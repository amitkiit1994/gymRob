    ...
    <article className="min-h-screen bg-primary-950">
      ...
      <header className="iron-bg iron-grain spark-corner-tl spark-corner-br relative pt-28 sm:pt-32 pb-14 sm:pb-20 overflow-hidden border-b-2 border-accent-800/60">
        <div className="relative container mx-auto px-4 sm:px-6 lg:px-8 max-w-5xl z-10">
          {/* Top tag — heavy industrial badge */}
          <div className="flex items-center gap-3 mb-8">
            <div className="inline-flex items-center gap-2.5 px-3 py-1.5 bg-black/70 border border-accent-700/70 rounded-sm">
              <span className="h-1.5 w-1.5 rounded-full bg-accent-500 animate-pulse" />
              <span className="font-serif italic text-[0.65rem] sm:text-xs tracking-[0.3em] uppercase text-accent-400 font-semibold">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-6 sm:gap-8 md:gap-10">
            ...
            <figure className="flex-shrink-0">
              <div className="iron-frame relative rounded-sm overflow-hidden bg-black">
                ...
                />
                {/* Rust tint overlay */}
                <div className="pointer-events-none absolute inset-0 bg-gradient-to-tr from-accent-900/20 via-transparent to-accent-700/10 mix-blend-overlay" />
              </div>
            </figure>
            ...
            <div className="flex-1 min-w-0">
              ...
              {/* Byline strip on metal */}
              <div className="flex flex-wrap items-center gap-x-4 gap-y-1.5 text-xs sm:text-sm text-accent-200/80 font-mono">
                <span className="font-bold tracking-[0.15em] uppercase text-accent-300">{feature.source}</span>
                <span className="h-1 w-1 rounded-full bg-accent-700" aria-hidden="true" />
                <time dateTime={feature.publishedDate} className="tracking-wider uppercase text-[0.65rem] sm:text-xs">
                  {feature.displayDate}
                </time>
                <span className="h-1 w-1 rounded-full bg-accent-700" aria-hidden="true" />
                <a
                  ...
                  className="inline-flex items-center gap-1 text-accent-400 hover:text-accent-200 transition-colors uppercase tracking-wider text-[0.65rem] sm:text-xs font-semibold"
                >
