...
export default function Nameplate({
  ...
  const base = {
    primary:
      'bg-gradient-to-b from-steel-scale via-steel-gunmetal to-furnace-ore ' +
      'border-2 border-rust-corrosion ' +
      'text-steel-polished ' +
    ...
    secondary:
      'bg-gradient-to-b from-furnace-plate to-furnace-base ' +
      'border-2 border-steel-scale ' +
      'text-steel-brushed ' +
      'shadow-[inset_0_1px_0_rgba(249,250,251,0.1),0_4px_0_-1px_#000,0_8px_18px_rgba(0,0,0,0.8)]',
    tag:
      'bg-gradient-to-b from-steel-gunmetal to-furnace-ore ' +
      'border border-rust-corrosion/80 ' +
      'text-rust-melt ' +
  ...
  return (
    <Component
      ...
      className={`
        ...
        font-painted uppercase tracking-wider select-none
        ...
        hover:border-rust-spark hover:text-steel-polished
        ${className}
      `}
      style={tiltStyle}
    >
...
function Rivet({ className = '', size = 'h-1.5 w-1.5' }: { className?: string; size?: string }) {
  return (
    <span
      aria-hidden="true"
      className={`${size} ${className} rounded-full bg-gradient-to-br from-steel-light via-steel-cast to-steel-gunmetal shadow-[inset_0_-1px_0_rgba(0,0,0,0.7),0_1px_1px_rgba(0,0,0,0.85)]`}
    />
  )
}
